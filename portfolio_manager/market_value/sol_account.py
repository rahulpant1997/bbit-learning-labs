...
class Account(AccountInterface):
    ...
    def remove_positions(self, securities: Set) -> None:
        for security_key in securities:
            if isinstance(security_key, SecurityInterface):
                self.positions.pop(security_key.get_name(), None)
            else:
                self.positions.pop(security_key, None)

    def get_current_market_value(self) -> float:
        return fsum(
            [pos.get_current_market_value() for pos in self.positions.values()]
        )

    def get_current_filtered_market_value(self, securities: Set) -> float:
        return fsum(
            [
                pos.get_current_market_value()
                for pos in self.get_positions(securities).values()
            ]
        )
